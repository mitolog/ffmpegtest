...
class Tutorial1 {
    ...
    */
    func saveAsPPM(frame: UnsafePointer<AVFrame>, filePath: String, width: Int, height: Int) -> Bool {
        ...
        // ヘッダーを書き込む
        let header = "P6\n\(width) \(height)\n255\n"
        fwrite(header.cStringUsingEncoding(NSUTF8StringEncoding)!, 1, Int(count(header)), file)
        ...
        for i in 0..<height {
            ...
            fwrite(y.baseAddress, 1, Int(width * 3), file)
        }
        if fclose(file) != 0 {
            return false
        }
        ...
        while av_read_frame(formatContext, packet) >= 0  {
            ...
            if packet.memory.stream_index == Int32(n) {
                ...
                if decodeVideo(codecContext, frame: frame, packet: packet) {
                    ...
                    // 100フレームごとに1つ保存
                    if ++i % 100 == 0 {
                        saveAsPPM(convertedFrame, filePath: "\(bundlePath)/frame\(i).ppm", width: Int(w), height: Int(h))
                    }
                }
            }
